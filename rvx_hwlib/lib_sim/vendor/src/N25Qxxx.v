...
`endif
 ...
 // at least 0.7s sec after, write/ read command must come on. (S. Lee) 
            $display("  [%0t ns] Command REAL execution begins: %0s.", $time, operation);
            if (operation=="Sector Erase") begin delay=erase_delay;
                 $display("SIM, shorted erase delay = %d ns", delay); end 
            `ifdef Stack512Mb

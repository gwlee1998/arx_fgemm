...
FAKEFILE* ffopen(const char* name, const char* mode)
{
  ...
  if(strcmp(mode, "r")==0)
  {
    assert_msg(fp!=NULL, "\nNo file exist: %s", name);
    fakefile_set_read_mode_(fp);
  }
  else if(strcmp(mode, "w")==0)
  {
    if(fp!=NULL)
      fakefile_free_rvx(fp);
...
static inline fakefile_chunk_t* fakefile_chunk_malloc()
{
...
static int ffputc_(int c, FAKEFILE* fp)
{
  if(fp->current==NULL)
  {
    fp->head = fakefile_chunk_malloc();
    fp->current = fp->head;
  }
  else if(fp->current->current_size==fp->current->size)
  {
    fakefile_chunk_t* chunk = fakefile_chunk_malloc();

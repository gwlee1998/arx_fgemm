...
static inline memory_block_info_t *__alloc_by_collector(size_t size)
{
  ...
  {
    block = head->prev;
    if ((block == NULL) || (block == head)) // if last element
    {
      block = head;
      HASH_DEL(reallocator, head);
    }
    else
...
void *malloc_rvx(size_t size)
{
  ...
  if (size > 0)
  {
    ...
    if (ptr == NULL)
    {
      if (extended_size <= SMALL_DATA_SIZE)
      {
        ptr = __alloc_to_sram(extended_size);
        if (ptr == NULL)
          ptr = __alloc_to_dram(extended_size);
      }
      else
      {
        ptr = __alloc_to_dram(extended_size);
...
void *malloc_rvx_with_assert(size_t size, const char *file, unsigned int line, const char *func)
{
  ...
  assert_must_with_info(ptr, "malloc fails", file, line, func);
  return ptr;
}
...
void free_rvx(void *ptr)
{
  ...
  if (head == NULL)
  {
    HASH_ADD_INT(reallocator, size, block);
    block->prev = block;
    block->next = block;
  }
  else
...
void test_memory_leak()
{
  ...
  {
    unsigned int diff = heap_dram_addr - previous;
    if(diff==0)
    {

...
ervp_task_wait_fx_t dca_mru_copy_part(ervp_mop_mapping_t *mop_mapping, const dca_mru_hwinfo_t *const hwinfo, const ErvpMatrixInfo *mi_info, ErvpMatrixInfo *mo_info, int num_row, int num_col, unsigned int option_value)
{
	...
	else
	{
		...
		task_wait_fx = dca_mru_wait_fx(hwinfo);
	}
	return task_wait_fx;
}

ervp_task_wait_fx_t dca_mru_transpose_part(ervp_mop_mapping_t *mop_mapping, const dca_mru_hwinfo_t *const hwinfo, const ErvpMatrixInfo *mi_info, ErvpMatrixInfo *mo_info, int num_row, int num_col, unsigned int option_value)
{
	ervp_task_wait_fx_t task_wait_fx = NULL;
	if (mop_option_has_postprocess(option_value))
	{
...
ervp_task_wait_fx_t dca_mru_fill_fixed(ervp_mop_mapping_t *mop_mapping, const dca_mru_hwinfo_t *const hwinfo, ErvpMatrixInfo *result, int32_t value)
{
	// printf_function();

	dca_mru_inst_t inst;
	inst.opcode = DCA_MRU_FILL;
	dca_init_matrix_info(&(inst.mi));
	dca_generate_matrix_info(result, &(inst.mo));

	cache_flush_smart(1, result->addr);
	// mmiox1_inst_wait_vacanct(hwinfo->mmiox_info);
	mmiox1_input_push(hwinfo->mmiox_info, &value, 1);
	mmiox1_inst_push(hwinfo->mmiox_info, &inst, 1, 0);
	return dca_mru_wait_fx(hwinfo);
}
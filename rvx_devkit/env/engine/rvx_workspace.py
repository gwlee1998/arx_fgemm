...
class RvxWorkspace():
  ...
  def app_compile_list(self, platform_name:str, app_name:str):
    ...
    if not compile_list_file.is_file():
      ...
      line_list.append('all:./rvx_src')
      line_list.append('all:./rvx_include')
  ...
  def __prepare_app_source(self, platform_name:str, app_name:str, target_imp_class:str, build_mode:str):
    ...
    compile_list = []
    for compile_target in compile_list_path.read_text().split('\n'):
      if compile_target:
        allowed_type, dir = compile_target.split(':')
        if allowed_type=='all' or allowed_type==target_imp_class:
          path = Path(dir)
          if not path.is_absolute():
            path = compile_list_path.parent / path
          compile_list.append(path)
    total_file_list = gather_file_list(compile_list, ['.c','.cpp','.S','.h','.a'])
  ...
  def app_compile(self, platform_name:str, app_name:str, target_imp_class:str, build_mode:str, is_incremental:bool):
    ...
    if target_file_list:
      # duplicate header
      if not new_include_path.is_dir():
        new_include_path.mkdir()
  ...
  def info_app_list(self, platform_name:str):
    assert platform_name
    app_base_path = self._get_app_base_path(platform_name)
  ...
  def platform_util(self, platform_name:str):
    ...
    if not target_makefile.parent.is_dir():
      target_makefile.parent.mkdir()
  ...
  def imp_fpga_dump(self, target_imp_class:str, imp_instance_path:Path):
    ...
    dump_type_file = dump_dir / 'dump_type.txt'
    dump_data_file = dump_dir / 'dump_data.bin'

    # script
    template_file = self._get_env_file('ocd', 'dump_memory.tcl.template')
    script_file = imp_instance_path / template_file.stem
    config_list = (('TYPE_FILE',str(PurePosixPath(dump_type_file))),
                  ('DATA_FILE',str(PurePosixPath(dump_data_file))),)
    ...
    if is_linux:
      cmd = make_cmd_sudo(f'chown {script_file.owner()}:{script_file.group()} {dump_type_file} {dump_data_file}', self.devkit.get_sudo_passwd())
      run_shell_cmd(cmd, imp_instance_path)

    # read dump_type
    dump_type = int(dump_type_file.read_text())
    ...
    if dump_type==0:
      ...
      cmd = f'{self.config.python3_cmd} {split_fakefiles_file} -i {dump_data_file} -o {dump_dir}'
      result = run_shell_cmd(cmd,imp_instance_path)
      print(result.stdout)
    elif dump_type==2:
      image_file = self.devkit.output_file
      if not image_file:
        image_file = dump_dir / 'dumped_image.jpg'
      copy_file(dump_data_file, image_file)
      print(f'Check {image_file.relative_to(imp_instance_path)}')
    else:
        assert 0
